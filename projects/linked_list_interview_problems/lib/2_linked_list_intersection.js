...
function linkedListIntersection(list1, list2) {
  ...
  //find the longer one and iterate until it has the same # of nodes left as the short one
  //continue iterating until one runs out and check if they are equal 

  let list1Length = getLinkedListLength(list1.head)
  let list2Length = getLinkedListLength(list2.head)
  
  let diff = Math.abs(list2Length - list1Length)
  
  let long = list1Length >= list2Length ? list1.head : list2.head
  let short = list1Length < list2Length ? list1.head : list2.head
...
var getLinkedListLength = function(list){
  counter = 0
