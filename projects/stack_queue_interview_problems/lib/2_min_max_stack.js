...
class Stack {
    ...
    push(val) {
        ...
        if (!this.minArr.length || val < this.minArr[this.minArr.length - 1].val){
            this.minArr.push(newNode)
        }
        if (!this.maxArr.length || val > this.maxArr[this.maxArr.length - 1].val){
    ...
    pop() {
        ...
        if (this.top.value === this.maxArr[this.maxArr.length - 1].value) {
            this.maxValueStorage.pop();
        }

        if (this.top.value === this.minArr[this.minArr.length - 1].value) {
    ...
    min() {
        return this.minArr[this.minArr.length - 1] || null;
    }

    max() {
        return this.maxArr[this.maxArr.length - 1] || null;
    }
}
