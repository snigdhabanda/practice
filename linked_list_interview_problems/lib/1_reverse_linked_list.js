...
// Let's code!
// -----------
//notes: iterate through the list and set each node's next pointer to the prev
//when you reach the tail, break and set the head to be the tail
//and the tail's next to be the previous 

function reverseLinkedList(linkedList) {
  // TODO: Implement the reverseLinkedList function!
  let node = linkedList.head //this will change
  let first = node //so store the head here; it will become the tail at the end 
  let prev = null
  let next = null
  ...
  while (next = node.next){
    node.next = prev //setting head to null; 
    prev = node //second node's pointer set to head 
    node = next //at the end node will be tail 
  }
  linkedList.head = node //tail 
  linkedList.head.next = prev //tail's next 
